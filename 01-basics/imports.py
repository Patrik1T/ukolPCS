...
print(f"\nAktuální datum a čas je: {aktualni_datum_a_cas}\n")

for i in range(datetime.now().year, datetime.now().year + 5):
    datum_velikonoc = easter(i)
    print(f"Velikonoční neděle v pěti letech bude v datu: {datum_velikonoc.strftime('%d.%m.%Y')}")
aktualni_rok = datetime.now().year

while True:
    datum_stedreho_dne = datetime(aktualni_rok, 12, 24)
    if datum_stedreho_dne.weekday() == 6:
        print(f"\nNejbližší rok, kdy bude Štědrý den v neděli: {aktualni_rok}\n")
        break
    aktualni_rok += 1