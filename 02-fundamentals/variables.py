...
# Pro typ je to příkaz type a pro identitu objektu to je id
...
'''
AND (&) provede bitový součin dvou čísel na úrovni bitů.
OR (|) provede bitový součet dvou čísel na úrovni bitů.
(^) provede bitový exkluzivní součet (XOR) dvou čísel na úrovni bitů.
NOT (~) invertuje všechny bity v čísle (1 na 0 a 0 na 1).
posun vlevo (<<) posune bity čísla doleva o určitý počet pozic.
posun vpravo (>>) posune bity čísla doprava o určitý počet pozic.
'''
jmeno = "PatrikTo"
binarni = ''.join('1' if char.isalpha() and char not in 'AEIOUaeiou' else '0' for char in jmeno)
myself_binary = "0b" + binarni

myself_decimal = int(myself_binary, 2)

shifted_binary = bin(myself_decimal >> 2)

hex_number = int("1A", 16)
result_decimal = hex_number & myself_decimal

print(f"Binární výsledek {jmeno}: {myself_binary}")
print(f"Desítková hodnota: {myself_decimal}")
print(f"Bitový posun vpravo: {shifted_binary}")
print(f"Bitový součin s 1A: {result_decimal}")
